...
#Ejercicio 6#
SELECT  per.apellido1, per.apellido2, per.nombre,d.nombre AS "Departamento" FROM profesor AS p
...
SELECT p.nombre, p.apellido1, p.apellido2, p.nif, asignatura.nombre AS asignatura, ce.anyo_inicio, ce.anyo_fin 
FROM alumno_se_matricula_asignatura AS aa
JOIN curso_escolar AS ce
ON aa.id_curso_escolar = ce.id
JOIN asignatura 
ON aa.id_asignatura = asignatura.id
JOIN persona AS p ON p.id = aa.id_alumno 
WHERE p.nif = '26902806M';

#Ejercicio 8#
SELECT  departamento.nombre AS 'departamentos'  FROM asignatura AS a
JOIN grado ON a.id_grado = grado.id
JOIN profesor ON a.id_profesor = profesor.id_profesor
JOIN departamento ON profesor.id_departamento = departamento.id
GROUP BY departamento.nombre;

#Ejercicio 9#
SELECT p.id, p.nombre, p.apellido1, ce.anyo_inicio, ce.anyo_fin, a.nombre AS 'Nombre_asignatura'  
FROM alumno_se_matricula_asignatura AS am
JOIN curso_escolar AS ce ON am.id_curso_escolar = ce.id
JOIN persona AS p ON am.id_alumno = p.id
JOIN asignatura AS a ON am.id_asignatura = a.id
WHERE ce.anyo_inicio = 2018 AND ce.anyo_fin = 2019;

#LEFT JOIN & RIGHT JOIN#
#Ejercicio 1#
SELECT persona.nombre, persona.apellido1, persona.apellido2, departamento.nombre AS 'Departamento', persona.tipo
FROM profesor
RIGHT JOIN persona ON persona.id = profesor.id_profesor
LEFT JOIN departamento ON profesor.id_departamento = departamento.id
WHERE tipo = 'profesor'
ORDER BY departamento.nombre, persona.apellido1, persona.nombre
;
...
#Ejercicio 3#
SELECT departamento.nombre, COUNT(id_profesor)
FROM profesor
JOIN departamento ON profesor.id_departamento = departamento.id
...
FROM asignatura
RIGHT JOIN grado ON grado.id = asignatura.id
GROUP BY grado.id 
ORDER BY COUNT(asignatura.id) DESC; 

#Ejercicio 6#
SELECT COUNT(a.id) AS 'NroMaterias', g.nombre 
...
#Ejercicio 7#
SELECT a.tipo, SUM(creditos)
FROM asignatura AS a
GROUP BY a.tipo;

#Ejercicio 8#
SELECT COUNT(am.id_alumno) AS 'AlumnosMatriculados', ce.anyo_inicio 
FROM alumno_se_matricula_asignatura AS am
RIGHT JOIN curso_escolar AS ce ON am.id_curso_escolar = ce.id
GROUP BY ce.anyo_inicio
;

#Ejercicio 9#
SELECT COUNT(asignatura.id) AS 'CantAsig.', persona.nombre, persona.apellido1, persona.apellido2 FROM profesor
...
#Ejercicio 11#
SELECT f.codigo AS 'CodigoFabricante' , p.nombre AS 'NombreProducto', precio 
